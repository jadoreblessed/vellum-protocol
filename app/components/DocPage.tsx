...
import theme from "./ExperienceTheme.module.css";
...
export default function DocPage({ index, eyebrow, title, intro, action = { label: "Open the app", href: "/app" }, facts, steps, dark = false }: DocPageProps) {
  return <main className={`doc-shell ${theme.documents} ${dark ? "doc-dark" : ""}`}>
    <header className="nav"><div className="container nav-inner"><Link href="/" className="wordmark">vellum<span>.</span></Link><nav className="nav-links"><Link href="/how-it-works">How it works</Link><Link href="/notes">Notes</Link><Link href="/protocol">Protocol</Link><Link href="/docs">Docs</Link></nav><Link href={action.href} className="button acid">{action.label}</Link></div></header>
