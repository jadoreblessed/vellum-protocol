...
export default function DocPage({ index, eyebrow, title, intro, action = { label: "Open the app", href: "/app" }, facts, steps, dark = false }: DocPageProps) {
  const visualByEyebrow: Record<string, string> = {
    OWNERSHIP: "/brand/vellum-archive-hall-v1.webp",
    COLLATERAL: "/brand/vellum-case-collateral-v1.webp",
    VESTING: "/brand/vellum-case-vesting-v1.webp",
    GATING: "/brand/vellum-case-access-v1.webp",
    ISSUANCE: "/brand/vellum-case-otc-v1.webp",
    MATURITY: "/brand/vellum-case-vesting-v1.webp",
    "CLAIMABLE STATE": "/brand/vellum-case-access-v1.webp",
    GUARANTEES: "/brand/vellum-case-collateral-v1.webp",
    "INSTRUMENT CLASSES": "/brand/vellum-case-otc-v1.webp",
  };
  const visual = visualByEyebrow[eyebrow] ?? "/brand/vellum-ownership-valley-v2.webp";
  return <main className={`doc-shell ${dark ? "doc-dark" : ""}`}>
    ...
    <section className="doc-hero container"><Image className="doc-hero-art" src={visual} alt="" width={1536} height={1024} priority /><div className="doc-hero-copy"><div className="eyebrow mono">{index} / {eyebrow}</div><h1 className="doc-title">{title}</h1><p>{intro}</p><Link className="button acid" href={action.href}>{action.label} <span>↗</span></Link></div><div className="doc-index mono"><span>VLM / {index}</span><span>READING NOTE</span></div></section>
