...
export default function DocPage({ index, eyebrow, title, intro, action = { label: "Open the app", href: "/app" }, facts, steps, dark = false }: DocPageProps) {
  return <main className={`doc-shell ${dark ? "doc-dark" : ""}`}>
    ...
    <section className="doc-facts container">{facts.map(([key, value]) => <div className="doc-fact" key={key}><span className="mono">{key}</span><strong>{value}</strong></div>)}</section>
    <section className="doc-body container"><div className="eyebrow mono">HOW IT MOVES</div><div className="doc-steps">{steps.map(([num, heading, body]) => <article className="doc-step" key={num}><span className="mono">{num}</span><h2>{heading}</h2><p>{body}</p></article>)}</div></section>
