...
export default function VellumExperience() {
  ...
  return <main className={s.experience} ref={root}>
    ...
    <section className={s.positions} id="positions">
      ...
      <div className={s.noteRail} aria-label="Example bearer notes" tabIndex={0}>{notes.map(note => <Link href="/app" className={s.note} key={note.ticker} style={{"--note-accent":note.color} as CSSProperties}><div className={s.noteHeader}><span>VELLUM / {note.number}</span></div><div className={s.noteVisual}><Image src={note.art} alt={`Abstract glass sculpture for ${note.name}`} fill sizes="(max-width:700px) 80vw, 400px" /><span className={s.tokenBadge}><Image src={note.image} width={58} height={58} alt="" /></span></div><div className={s.noteIdentity}><h3>{note.name}</h3><span>{note.ticker}</span></div><strong className={s.noteAmount}>{note.amount}</strong><div className={s.noteFacts}><span>Lock term<b>{note.term}</b></span><span>Instrument<b>Bearer note</b></span></div><div className={s.noteBottom}><span>EXAMPLE POSITION</span><span>Make it yours</span></div></Link>)}</div>
    </section>
    ...
    <footer className={s.footer}><div className={s.footerScene}><Image src="/brand/vellum-sky-hero.webp" alt="" fill sizes="100vw" /><div className={s.footerShade}/><div className={s.footerColumns}><div className={s.footerBrandCard} aria-label="Vellum"><Image className={s.footerBrandSky} src="/brand/vellum-sky-hero.webp" alt="" fill sizes="200px" /><span className={s.footerBrandGlyph}><Image src="/brand/vellum-mark.webp" alt="" width={108} height={108} /></span></div><nav aria-label="Footer"><span>MENU</span><a href="#hero-title">Home</a><Link href="/docs">Docs</Link><a href="#how">How it works</a><Link href="/app">Open app</Link></nav><nav aria-label="Social links"><span>SOCIAL</span><a href="https://x.com/VellumRH" target="_blank" rel="noreferrer">Twitter / X</a></nav></div><div className={s.footerMarquee} aria-label="Vellum"><div><span>VELLUM®</span><span>VELLUM®</span><span>VELLUM®</span></div></div></div></footer>
  </main>;
}
