...
export default function VellumExperience() {
  ...
  return <main className={s.experience} ref={root}>
    ...
    <section className={s.positions} id="positions">
      ...
      <div className={s.noteRail} aria-label="Example bearer notes" tabIndex={0}>{notes.map(note => <Link href="/app" className={s.note} key={note.ticker} style={{"--note-accent":note.color} as CSSProperties}><div className={s.noteHeader}><span>VELLUM / {note.number}</span></div><div className={s.noteVisual}><Image src={note.art} alt={`${note.name} glass coin`} fill sizes="(max-width:700px) 80vw, 400px" /></div><div className={s.noteIdentity}><h3>{note.name}</h3><span>{note.ticker}</span></div><strong className={s.noteAmount}>{note.amount}</strong><div className={s.noteFacts}><span>Lock term<b>{note.term}</b></span><span>Instrument<b>Bearer note</b></span></div><div className={s.noteBottom}><span>EXAMPLE POSITION</span><span>Make it yours</span></div></Link>)}</div>
    </section>
